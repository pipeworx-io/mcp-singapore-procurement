...
`tools/list` at `https://gateway.pipeworx.io/singapore-procurement/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/singapore_search_awards \
  -H 'Content-Type: application/json' \
  -d '{"query":"cloud","limit":20}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/singapore_search_awards`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "singapore-procurement": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-singapore-procurement"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-singapore-procurement
```
